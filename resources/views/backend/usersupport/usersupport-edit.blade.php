...
@section('title', 'User Support EditPage')
@section('content')

<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-sm-4">
        <h2 class="mt-3 mb-2">Edit User Support</h2>
        <ol class="breadcrumb">
        ...
        <div class="col-lg-12">
            <div class="ibox">
                <div class="ibox-content">
                    <form action="{{route('usersupport.update',$usersupp->id)}}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="form-group row">
                            <label class="col-sm-2 col-form-label"><strong>Title:</strong></label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control" name="usersupp_name" placeholder="Enter Title"
                                    value="{{$usersupp->name}}" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <select class="form-control" name="product_id" required>
                                    ...
                                    <option value="{{$item->id}}" {{ $item->id == $usersupp->product_id ?'selected' : ''
                                        }}>
                                        {{$item->name}}
                                    </option>
                                    @endforeach
                            ...
                            <div class="col-sm-10">
                                <div class="summernote">
                                    <textarea id="summernote" class="form-control mt-2" cols="20" rows="5"
                                        placeholder="Enter Body" name="usersupp_des"
                                        required>{{$usersupp->description}}</textarea>
