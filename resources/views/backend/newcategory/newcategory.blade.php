...
@section('title', 'News category Page')
@section('content')
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-sm-4">
        <h2 class="mt-3 mb-2">All News Category</h2>
        <ol class="breadcrumb">
    ...
    <div class="col-sm-8">
        <div class="form-group text-lg-start float-lg-right mt-5">
            @can('create category')
            <a href="{{route('new-category.create')}}" class="btn btn-dark m-t-n-xs">Create New Category</a>
            @endcan
        </div>
    </div>
</div>
            ...
            <div class="ibox">
                <div class="ibox-content">
                    <table class="footable table table-stripped toggle-arrow-tiny" data-page-size="15">
                        <thead>
                            <tr>
                                <th data-toggle="true">Project Category Name</th>
                                <th class="text-right" data-sort-ignore="true">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            @if (count($newcategory) > 0)
                            @foreach ($newcategory as $newcat)
                            <tr>
                                <td>
                                    {{$newcat->name}}
                                </td>
                                <td class="text-right">
                                    <div class="btn-group">
                                        ...
                                        <button class="btn-white btn btn-sm">
                                            <a href="{{route('new-category.edit',$newcat->id)}}"
                                                class="text-dark">Edit</a>
                                        </button>
                                        @endcan
                                        @can('delete category')
                                        <button class="btn btn-white btn-sm text-dark"
                                            onclick="deleteForm('{{route('new-category.delete')}}', {{$newcat->id}})">delete</button>
                                        @endcan
                                    </div>
                                </td>
                            </tr>
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="6">
                                    {{ $newcategory->links('backend.layouts.pagination_ui') }}
                                </td>
                            </tr>
                        </tfoot>
                    </table>
