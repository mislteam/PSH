            ...
            <div class="row">
                <div class="col text-center">
                    <ul class="breadcrumb d-flex justify-content-center text-4-5 font-weight-medium mb-2">
                        ...
                        <li class="text-color-primary active">News</li>
                    </ul>
                    <h1 class="text-color-light font-weight-bold text-10">Lorem ipsum dolor sit a met</h1>
                </div>
            </div>
        </div>
    </section>
    <!-- END BANNER -->
    <!-- START OUR BLOG DETAIL-->
    <div class="container py-5 my-4">
        <div class="container">
            <div class="row pb-4">
                <div class="col-lg-8 mb-5 mb-lg-0 appear-animation" data-appear-animation="fadeInUpShorterPlus"
                    data-appear-animation-delay="200">
                    <h2 class="text-color-dark font-weight-bold text-10 mb-2"><em>{{$new_detail->name}}</em></h2>
                    <ul
                        ...
                        <li class="font-weight-semibold text-color-dark mb-0">
                            ...
                            {{$new_detail->newcategory->name}}
                        </li>
                    </ul>
                    <img src="{{asset('storage/new_feature_image/'. $new_detail->feature_image) }}"
                        class="img-fluid mb-3">
                    <p class="mb-0">{!!$new_detail->description!!}</p>
                    {{-- <div class="row">
                ...
                <div class="col-lg-4 position-relative">
                    <aside class="sidebar" data-plugin-sticky
                        data-plugin-options="{'minWidth': 1000, 'containerSelector': '.container', 'padding': {'top': 120}}">
                        <div class="card border-0 border-radius-0 custom-box-shadow-1 mb-5">
                            <div class="card-body p-4 my-3">
                                <h3 class="text-transform-none font-weight-bold mb-4">News Categories</h3>
                                <ul class="custom-list-style-1 list list-unstyled font-weight-bold">
                                    @foreach ($newcategory as $item)
                                    <li class="{{ $item->id == $new_detail->new_category_id ? 'active' : '' }}">
                                        <a href="#"
                                            class="text-decoration-none text-color-hover-primary d-flex {{ $item->id == $new_detail->new_category_id ? 'active' : 'text-dark' }}">
                                            {{$item->name}}</a>
                                    </li>
                                    @endforeach
                                </ul>
